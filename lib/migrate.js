...
const reverseSequelizeColType = function(col, prefix = 'Sequelize.') 
{
    ...
    switch (attrName) {
        ...
        case DataTypes.INTEGER.key: {
            let ret = attrName.replace(DataTypes.DOUBLE.key, "DOUBLE");
            if (options.length)
            {
                ret += '('+options.length;
                if (options.decimals)
        ...
        case DataTypes.ARRAY.key:
            console.log('Ok we have got(this could be a tricky part) ' + attrName + ' with attrObj: ' + attrObj + ' and col.seqType: ' + col.seqType);
            ...
            switch (_type) {
                ...
                case 'FLOAT[]':
                    ...
                    break;
                case 'VARCHAR(255)[]':
                    arrayType = prefix + 'STRING';
                    break;
                default:
                    console.warn(attrName + ' type with subtype' + _type + ' not supported, so subtype was casted to STRING');
                    arrayType = prefix + 'STRING';
                    break;
            }
